...
const { blackList } = getLimiters();
  ...
  if (Number.isNaN(Number(result.payload.sub)) || typeof raw.visitor !== 'number') {
    ...
    strangeThings(session, canary, req.ip!, req.get('User-Agent')!, false);
      ...
      if (!valid && reqMFA) {
        log.info({token: session,valid, reason, reqMFA, userId, visitorId},`mfa is triggered`)
        const mfa = await sendTempMfaLink(
          ...
          visitor: visitorId!
            ...
            session)
            if (!mfa) { 
              log.warn({token: session,valid, reason, reqMFA, userId, visitorId},`mfa error 500`)
            res.status(500).json({ error: 'Could not send MFA code, try again later' });
            return;
            }
            log.info({token: session,valid, reason, reqMFA, userId, visitorId},`A login link has been sent to the user`)
            res.status(202).json({ mfa: true, message: 'A login link has been sent to your email.' });
            return;
         }

     if (!valid && !reqMFA) {
       log.info({token: session,valid, reason, reqMFA, userId, visitorId},`Relogin is required`)
