...
import mysql2 from 'mysql2/promise';
  ...
  describe('consumeAndVerifyRefreshToken', () => {
    ...
    it('should consume a valid token successfully', async ({testUserId, mainPool}) => {
    ...
    it('should detect and prevent token reuse attacks', async ({mainPool}) => {
    ...
    it('should handle revoked tokens properly', async ({mainPool}) => {
    ...
    it('should maintain transaction integrity', async ({mainPool}) => {
      const result = await consumeAndVerifyRefreshToken(validToken.raw);
      expect(result.valid).toBe(true);
